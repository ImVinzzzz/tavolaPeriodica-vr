...
const ElementModal: React.FC<ElementModalProps> = ({ element, onClose }) => {
  ...
  const isUnstableNoStdWeight =
    element.isRadioactive && (element.number === 43 || element.number === 61 || element.number >= 84);
  const formattedAtomicWeight = isUnstableNoStdWeight
    ? "[" + element.atomicWeight + "] u"
    : element.atomicWeight + " u";
  const isTheoreticalConfig = element.electronConfiguration.startsWith("*");
  const cleanConfig = isTheoreticalConfig
    ? element.electronConfiguration.slice(1)
            ...
            <div className="p-6 sm:p-8 grid sm:grid-cols-[auto_1fr] gap-6">
              ...
              <div className="min-w-0">
                ...
                <div className="grid sm:grid-cols-2 gap-x-6">
                  <div>
                    <DataRow label="Peso atomico" value={formattedAtomicWeight} />
                    <DataRow
                      label="Energia di ionizzazione"
                      value={element.ionizationEnergy !== null ? element.ionizationEnergy + " kJ/mol" : "N/D (non misurata)"}
                    />
                    <DataRow
                      label="Elettronegatività"
                      value={element.electronegativity !== null ? String(element.electronegativity) : "N/D (non definita)"}
                    />
                  ...
                  <div>
                    <DataRow
                      ...
                      value={element.atomicRadius !== null ? element.atomicRadius + " pm" : "N/D (non misurato)"}
                    />
                    <DataRow
                      label="Densità"
                      value={element.density !== null ? element.density + " g/cm³" : "N/D (non misurata)"}
                    />
                    <DataRow label="Anno di scoperta" value={String(element.yearDiscovered)} />
                ...
                <div className="mt-3 flex items-start gap-2 text-xs text-white/60 bg-white/5 rounded-lg p-3 border border-white/10">
                  ...
                  <div>
                    <span className="text-white/45 uppercase tracking-wide text-[10px] block mb-0.5">
                      {"Configurazione elettronica" + (isTheoreticalConfig ? " (predetta)" : "")}
                    </span>
                    <span className="font-mono text-white/85">{cleanConfig}</span>
                  </div>
                </div>
